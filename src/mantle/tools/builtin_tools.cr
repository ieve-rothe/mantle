...
module Mantle::Tools
  ...
  class BuiltinToolExecutor
    ...
    private def execute_notify_send(arguments : Hash(String, JSON::Any)) : String
      ...
      # Sanitize message: strip control characters, backticks, and ANSI escapes
      # to prevent terminal manipulation or command execution via vulnerable notification daemons.
      clean_message = message.gsub(/[`\x00-\x1F\x7F]/, "")

      # Validate message to prevent argument injection and malicious payloads
      if clean_message.strip.starts_with?("-")
        return {
          success: false,
